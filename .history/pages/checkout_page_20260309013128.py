...
class CheckoutProcess:
    def __init__(self, page:Page):
        ...
        self.checkout = page.get_by_role("button", name="Check out")
        self.account = page.get_by_role("textbox", name="Email")
        self.card_number = page.frame_locator('iframe[name^="card-fields-number-"]').get_by_role("textbox", name="Card number")
        self.exp_date = page.frame_locator('iframe[name^="card-fields-expiry-"]').get_by_role("textbox", name="Expiration date (MM / YY)")
        self.sec_code = page.frame_locator('iframe[name^="card-fields-verification_value-"]').get_by_role("textbox", name="Security code")
        self.card_name = page.frame_locator('iframe[name^="card-fields-name-"]').get_by_role("textbox", name="Name on card")
    ...
    def pay_now(self):
        expect(self.pay).to_be_visible(timeout=15000)
        expect(self.pay).to_be_enabled(timeout=15000)
        print("Pay button visible:", self.pay.is_visible())
        print("Pay button enabled:", self.pay.is_enabled())
        self.pay.click()
